...
exports.getPrevServices = async function (userId) {
    connection = await pool.getConnection(async (conn) => conn);
    const serviceInfo = {};
    ...
    try {
        await connection.beginTransaction();
        //1 .Dao: serviceId, petSitterId,startTime~endTime,isLike
        
        const serviceDetail = await userDao.getPrevServicesDetail(connection, userId);
        serviceInfo.info = serviceDetail;
        //2. pet정보
        for (i = 0; i < serviceInfo.info.length; i++){
            const servicePetList = await userDao.getServicePets(connection, serviceInfo.info[i].serviceId);
            serviceInfo.info[i].pets = servicePetList;
        }
